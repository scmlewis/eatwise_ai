...
def dashboard_page():
    ...
    with quick_stats_col1:
        ...
        with water_btn_col1:
            if st.button("➕ Add", key="add_water_btn"):
        ...
        with water_btn_col2:
            if st.button("➖ Remove", key="remove_water_btn"):
                if current_water > 0:
                    if db_manager.log_water(st.session_state.user_id, -1, today):
        ...
        with water_btn_col3:
            if st.button("🏁 Complete", key="fill_water_btn", disabled=(current_water >= water_goal)):
...
def meal_logging_page():
    ...
    if recent_meals:
        ...
        with col2:
            if st.button("➕ Quick Add", key="quick_add_btn"):
    ...
    with tab1:
        ...
        if "meal_analysis" in st.session_state:
            ...
            # Save meal
            if st.button("Save This Meal", key="text_save_btn"):
                meal_data = {
                    "user_id": st.session_state.user_id,
    ...
    with tab2:
        ...
        if "photo_analysis" in st.session_state:
            ...
            # Save meal
            if st.button("Save This Meal", key="save_photo_meal"):
                meal_data = {
                    "user_id": st.session_state.user_id,
    ...
    with tab3:
        ...
        if st.button("📥 Analyze & Save All Meals", key="batch_save_btn"):
            total_saved = 0
            total_failed = 0
...
def insights_page():
    ...
    with export_col3:
        if st.button("📊 Share Weekly Summary"):
            end_date = date.today()
            start_date = end_date - timedelta(days=7)
            weekly_meals = db_manager.get_meals_in_range(st.session_state.user_id, start_date, end_date)
...
def meal_history_page():
    ...
    with col3:
        ...
        if st.button("🔍 Search", key="search_meals_btn"):
    ...
    if st.session_state.get("search_triggered", False) or date.today() == end_date:
        ...
        for meal in paginated_meals:
            ...
            with col2:
                if st.button("Edit", key=f"edit_hist_{meal['id']}"):
                    st.session_state[f"edit_meal_id_{meal['id']}"] = True
            
            with col3:
                if st.button("Duplicate", key=f"dup_hist_{meal['id']}"):
                    st.session_state[f"dup_meal_id_{meal['id']}"] = True
            
            with col4:
                if st.button("Delete", key=f"delete_hist_{meal['id']}"):
            ...
            if st.session_state.get(f"dup_meal_id_{meal['id']}", False):
                ...
                with dup_col1:
                    if st.button("✅ Duplicate Meal", key=f"confirm_dup_{meal['id']}"):
                        meal_data = {
                            "user_id": st.session_state.user_id,
                            "meal_name": meal.get('meal_name', 'Unknown'),
                            "description": meal.get('description', ''),
                ...
                with dup_col2:
                    if st.button("❌ Cancel", key=f"cancel_dup_{meal['id']}"):
                        st.session_state[f"dup_meal_id_{meal['id']}"] = False
                        st.rerun()
            ...
            if st.session_state.get(f"edit_meal_id_{meal['id']}", False):
                ...
                with st.form(f"edit_hist_form_{meal['id']}"):
                    ...
                    with btn_col1:
                        if st.form_submit_button("💾 Save Changes", key=f"save_hist_{meal['id']}"):
                            updated_meal = {
                                "meal_name": meal_name,
                                "meal_type": meal_type,
                    ...
                    with btn_col2:
                        if st.form_submit_button("❌ Cancel", key=f"cancel_hist_{meal['id']}"):
                            st.session_state[f"edit_meal_id_{meal['id']}"] = False
                            st.rerun()
        ...
        if total_pages > 1:
            ...
            with pag_col1:
                if st.button(f"⬅️ Previous ({current_page}/{total_pages})", key="prev_bottom", disabled=(st.session_state.pagination_page == 0)):
                    st.session_state.pagination_page -= 1
                    st.rerun()
            
            with pag_col2:
                if st.button(f"Next ({current_page}/{total_pages}) ➡️", key="next_bottom", disabled=(st.session_state.pagination_page >= total_pages - 1)):
                    st.session_state.pagination_page += 1
                    st.rerun()
...
def coaching_assistant_page():
    ...
    with page_container:
        ...
        with st.form(key="chat_form", clear_on_submit=True):
            ...
            with form_col3:
                clear_button = st.form_submit_button("🔄 Clear", key="clear_coaching_btn")
                if clear_button:
                    st.session_state.coaching_conversation = []
                    st.rerun()
...
def restaurant_analyzer_page():
    ...
    with tab_text:
        ...
        if st.button("🔍 Analyze Menu", type="primary", key="analyze_text_btn"):
    ...
    with tab_photo:
        ...
        if uploaded_file is not None:
            ...
            if "extracted_menu_text" in st.session_state:
                ...
                if st.button("✅ Analyze Extracted Menu", type="primary"):
                    with st.spinner("🤖 Analyzing menu with AI..."):
                        # Get today's nutrition
                        today_nutrition = db_manager.get_daily_nutrition_summary(
                            st.session_state.user_id, date.today()
