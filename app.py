...
def login_page():
    ...
    with col2:
        ...
        with tab1:
            ...
            if st.button("Login", key="login_btn", width=None):
        ...
        with tab2:
            ...
            if st.button("Sign Up", key="signup_btn", width=None):
...
def dashboard_page():
    ...
    with quick_stats_col1:
        ...
        with water_btn_col1:
            if st.button("➕ Add", width=None, key="add_water_btn"):
        ...
        with water_btn_col2:
            if st.button("➖ Remove", width=None, key="remove_water_btn"):
                if current_water > 0:
                    if db_manager.log_water(st.session_state.user_id, -1, today):
        ...
        with water_btn_col3:
            if st.button("🏁 Complete", width=None, key="fill_water_btn", disabled=(current_water >= water_goal)):
    ...
    with breakdown_col1:
        ...
        if daily_nutrition['protein'] > 0 or daily_nutrition['carbs'] > 0 or daily_nutrition['fat'] > 0:
            ...
            st.plotly_chart(fig_macro, width=None, config={'displayModeBar': False})
        else:
...
def meal_logging_page():
    ...
    if recent_meals:
        ...
        with col2:
            if st.button("➕ Quick Add", width=None, key="quick_add_btn"):
    ...
    with tab1:
        ...
        if st.button("Analyze Meal", width=None):
            if meal_description:
        ...
        if "meal_analysis" in st.session_state:
            ...
            # Save meal
            if st.button("Save This Meal", width=None, key="text_save_btn"):
                meal_data = {
                    "user_id": st.session_state.user_id,
    ...
    with tab2:
        ...
        if uploaded_file:
            ...
            if st.button("Analyze Photo", width=None):
        ...
        if "photo_analysis" in st.session_state:
            ...
            # Save meal
            if st.button("Save This Meal", width=None, key="save_photo_meal"):
                meal_data = {
                    "user_id": st.session_state.user_id,
    ...
    with tab3:
        ...
        if st.button("📥 Analyze & Save All Meals", width=None, key="batch_save_btn"):
            total_saved = 0
            total_failed = 0
...
def analytics_page():
    ...
    with col1:
        if st.button("Last 7 days", width=None):
            st.session_state.analytics_days = 7
            st.rerun()
    
    with col2:
        if st.button("Last 2 weeks", width=None):
            st.session_state.analytics_days = 14
            st.rerun()
    
    with col3:
        if st.button("Last 30 days", width=None):
    ...
    st.plotly_chart(fig_cal, width=None)
    ...
    st.plotly_chart(fig_macro, width=None)
...
def insights_page():
    ...
    if st.button("🤖 Generate Meal Recommendations", width=None):
    ...
    if st.button("Generate 7-Day Meal Plan", width=None):
    ...
    if st.button("🤖 Analyze Health Insights", width=None):
        with st.spinner("🤖 Analyzing your eating patterns..."):
            ...
            if insights:
                ...
                with col1:
                    if st.button("📋 Copy to Clipboard", width=None):
                ...
                with col2:
                    if st.button("🔗 Share as Text", width=None):
    ...
    if weekly_meals:
        ...
        if meal_type_counts:
            ...
            with dist_col1:
                ...
                st.plotly_chart(fig_dist, width=None)
    ...
    with export_col1:
        if st.button("📄 Export as CSV", width=None):
    ...
    with export_col2:
        if st.button("📋 Export Report", width=None):
    ...
    with export_col3:
        if st.button("📊 Share Weekly Summary", width=None):
            end_date = date.today()
            start_date = end_date - timedelta(days=7)
            weekly_meals = db_manager.get_meals_in_range(st.session_state.user_id, start_date, end_date)
...
def meal_history_page():
    ...
    with col3:
        ...
        if st.button("🔍 Search", width=None, key="search_meals_btn"):
    ...
    if st.session_state.get("search_triggered", False) or date.today() == end_date:
        ...
        for meal in paginated_meals:
            ...
            with col2:
                if st.button("Edit", key=f"edit_hist_{meal['id']}", width=None):
                    st.session_state[f"edit_meal_id_{meal['id']}"] = True
            
            with col3:
                if st.button("Duplicate", key=f"dup_hist_{meal['id']}", width=None):
                    st.session_state[f"dup_meal_id_{meal['id']}"] = True
            
            with col4:
                if st.button("Delete", key=f"delete_hist_{meal['id']}", width=None):
            ...
            if st.session_state.get(f"dup_meal_id_{meal['id']}", False):
                ...
                with dup_col1:
                    if st.button("✅ Duplicate Meal", width=None, key=f"confirm_dup_{meal['id']}"):
                        meal_data = {
                            "user_id": st.session_state.user_id,
                            "meal_name": meal.get('meal_name', 'Unknown'),
                            "description": meal.get('description', ''),
                ...
                with dup_col2:
                    if st.button("❌ Cancel", width=None, key=f"cancel_dup_{meal['id']}"):
                        st.session_state[f"dup_meal_id_{meal['id']}"] = False
                        st.rerun()
            ...
            if st.session_state.get(f"edit_meal_id_{meal['id']}", False):
                ...
                with st.form(f"edit_hist_form_{meal['id']}"):
                    ...
                    with btn_col1:
                        if st.form_submit_button("💾 Save Changes", width=None, key=f"save_hist_{meal['id']}"):
                            updated_meal = {
                                "meal_name": meal_name,
                                "meal_type": meal_type,
                    ...
                    with btn_col2:
                        if st.form_submit_button("❌ Cancel", width=None, key=f"cancel_hist_{meal['id']}"):
                            st.session_state[f"edit_meal_id_{meal['id']}"] = False
                            st.rerun()
        ...
        if total_pages > 1:
            ...
            with pag_col1:
                if st.button(f"⬅️ Previous ({current_page}/{total_pages})", key="prev_bottom", disabled=(st.session_state.pagination_page == 0), width=None):
                    st.session_state.pagination_page -= 1
                    st.rerun()
            
            with pag_col2:
                if st.button(f"Next ({current_page}/{total_pages}) ➡️", key="next_bottom", disabled=(st.session_state.pagination_page >= total_pages - 1), width=None):
                    st.session_state.pagination_page += 1
                    st.rerun()
...
def profile_page():
    ...
    with tab1:
        ...
        if not user_profile:
            ...
            with st.form("health_profile_form"):
                ...
                if st.form_submit_button("Save Profile", width=None):
                    profile_data = {
                        "user_id": st.session_state.user_id,
                        "full_name": full_name,
                        "age_group": age_group,
            ...
            with st.form("update_profile_form"):
                ...
                if st.form_submit_button("Update Profile", width=None):
                    update_data = {
                        "full_name": full_name,
                        "age_group": age_group,
                        "gender": gender,
    ...
    with tab2:
        ...
        with st.form("change_password_form"):
            ...
            if st.form_submit_button("Change Password", width=None):
                # Validate inputs
...
def coaching_assistant_page():
    ...
    with page_container:
        ...
        with st.form(key="chat_form", clear_on_submit=True):
            ...
            with form_col2:
                send_button = st.form_submit_button("📤 Send", width=None)
            
            with form_col3:
                clear_button = st.form_submit_button("🔄 Clear", width=None, key="clear_coaching_btn")
                if clear_button:
                    st.session_state.coaching_conversation = []
                    st.rerun()
...
def restaurant_analyzer_page():
    ...
    with tab_text:
        ...
        if st.button("🔍 Analyze Menu", width=None, type="primary", key="analyze_text_btn"):
    ...
    with tab_photo:
        ...
        if uploaded_file is not None:
            ...
            with col2:
                st.markdown("")  # Spacing
                if st.button("📸 Extract Text from Photo", width=None, type="primary"):
            ...
            if "extracted_menu_text" in st.session_state:
                ...
                if st.button("✅ Analyze Extracted Menu", width=None, type="primary"):
                    with st.spinner("🤖 Analyzing menu with AI..."):
                        # Get today's nutrition
                        today_nutrition = db_manager.get_daily_nutrition_summary(
                            st.session_state.user_id, date.today()
...
def main():
    ...
    if not is_authenticated():
        ...
        if st.session_state.user_email:
            ...
            # Logout button
            if st.sidebar.button("🚪 Logout", width=None, key="logout_btn"):
